...
def build_quote_totals(quote, payload):
    ...
    for job in jobs:
        labor_hours = float(job.get("labor_hours", 0) or 0)
        parts_quality = float(job.get("parts_quality", 0) or 0)
        subtotal_labor += labor_hours * labor_rate
        subtotal_parts += parts_quality
...
def save_quote():
    ...
    labor_rate = float(request.form.get("labor_rate") or 0)
    ...
    job_parts_oem = request.form.getlist("job_parts_oem[]")
    job_parts_quality = request.form.getlist("job_parts_quality[]")
    job_parts_economy = request.form.getlist("job_parts_economy[]")
    ...
    max_len = max(
        ...
        len(job_labor_hrs),
        len(job_parts_oem),
        len(job_parts_quality),
        len(job_parts_economy),
        len(job_notes),
    ) if any([job_desc, job_labor_hrs, job_parts_oem, job_parts_quality, job_parts_economy, job_notes]) else 0

    for i in range(max_len):
        desc = job_desc[i].strip() if i < len(job_desc) else ""
        labor_hours = float(job_labor_hrs[i] or 0) if i < len(job_labor_hrs) else 0
        parts_oem = float(job_parts_oem[i] or 0) if i < len(job_parts_oem) else 0
        parts_quality = float(job_parts_quality[i] or 0) if i < len(job_parts_quality) else 0
        parts_economy = float(job_parts_economy[i] or 0) if i < len(job_parts_economy) else 0
        notes = job_notes[i].strip() if i < len(job_notes) else ""

        if not desc and labor_hours == 0 and parts_oem == 0 and parts_quality == 0 and parts_economy == 0 and not notes:
            continue
...
def convert_invoice(token):
    ...
    for idx, tier in approved_map.items():
        ...
        labor_total += float(job.get("labor_hours", 0) or 0) * float(quote["labor_rate"] or 0)

        if tier == "oem":
            parts_total += float(job.get("parts_oem", 0) or 0)
        elif tier == "economy":
            parts_total += float(job.get("parts_economy", 0) or 0)
        else:
            parts_total += float(job.get("parts_quality", 0) or 0)
